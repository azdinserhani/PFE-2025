...
export default function CourseSections({ module }) {
      ...
      >
        {module?.lessons?.map((lesson, index) => (
          <motion.div
            ...
            style={{ 
              borderBottom: index < module?.lessons?.length - 1 ? `1px solid ${theme.border}30` : 'none',
              color: lesson.locked ? theme.secondary : theme.text,
              cursor: lesson.locked ? 'not-allowed' : 'pointer'
            }}
            whileHover={{ 
              backgroundColor: lesson.locked ? 'transparent' : `${theme.primary}10` 
            }}
          >
            <div className="flex items-center gap-3">
              <div className="w-7 h-7 rounded-full flex items-center justify-center"
                style={{ 
                  backgroundColor: lesson.locked ? `${theme.secondary}20` : `${theme.primary}20`,
            ...
            <span className="px-2 py-1 rounded text-xs"
              style={{ 
                backgroundColor: lesson.locked ? `${theme.secondary}15` : `${theme.primary}15`,
                color: lesson.locked ? theme.secondary : theme.primary 
              }}
            >
              {lesson.duration}
